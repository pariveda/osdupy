...
class TestLegalService_WithSideEffects(TestOsduServiceBase):
    ...
    def test_001_create_legaltag(self):
        test_data_file = 'tests/test_data/test_create_legaltag.json'
        with open(test_data_file, 'r') as _file:
            legaltag_to_store = json.load(_file)

        result = self.osdu.legal.create_legaltag(legaltag_to_store)

        self.assertIsNotNone(result["name"])

    def test_002_get_legaltag(self):
        legaltag = self.osdu.legal.get_legaltag("osdu-testing-legal-tag-plz-delete")
    ...
    def test_004_update_legaltag(self):
        test_data_file = 'tests/test_data/test_update_legaltag.json'
        with open(test_data_file, 'r') as _file:
            legaltag_to_store = json.load(_file)

        result = self.osdu.legal.update_legaltag(legaltag_to_store)

        self.assertEqual(result['description'], legaltag_to_store['description'])
    
    def test_005_delete_legaltag(self):
        tag_was_deleted = self.osdu.legal.delete_legaltag("osdu-testing-legal-tag-plz-delete")
